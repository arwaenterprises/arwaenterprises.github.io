...
function showAddClientModal() {
    ...
    document.getElementById('passwordHelp').textContent = 'Initial password for admin login';
    
    // Hide reset password button for new clients
    document.getElementById('resetPasswordSection').style.display = 'none';
    document.getElementById('adminSectionNote').textContent = 'Set login credentials for this client\'s admin user';
...
async function editClient(clientId) {
    try {
        ...
        document.getElementById('adminSectionNote').textContent = 'Update login credentials (password optional)';
    ...
    try {
        ...
        if (isNew) {
            ...
            const { error: emailError } = await supabaseClient
                .from('users')
                .update({ username: contactEmail.toLowerCase() })
                .eq('client_id', clientId)
                .eq('role', 'super_admin');
